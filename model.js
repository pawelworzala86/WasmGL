...
const obj = fs.readFileSync('./models/worzala/worzala.obj').toString()
...
for(let model of meshes){
   ...
   /* addInt64([BigInt(model.position.length)])
        ...
        var idx = 0
    for(let index = 0; index<model.position.length; index+=3){

        //console.log(model.position[(idx*3)+0])
        data.push(model.position[(idx*3)+0])
        data.push(model.position[(idx*3)+1])
        data.push(model.position[(idx*3)+2])

        data.push(model.normal[(idx*3)+0])
        data.push(model.normal[(idx*3)+1])
        data.push(model.normal[(idx*3)+2])

        data.push(model.coord[(idx*2)+1])
        data.push(model.coord[(idx*2)+2])
...
fs.writeFileSync('./src/worzala.ts',`let quad_data: StaticArray<f32> = [
